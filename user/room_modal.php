				...
				<div class="container-fluid">
					...
					<div class="form-group input-group">
						...
						<select style="width:350px;" class="form-control" name="user">
                            ...
                            $obj = new dboperation();
								$mem=array();
								$um="select * from `chat_member` where chatroomid='$id'";
                            	$result=$obj->executequery($um);
								while($umrow=mysqli_fetch_array($result)){
									$mem[]=$umrow['userid'];
								}
								$users=implode($mem, "', '"); 
								
								$u="select * from `user` where userid not in ('".$users."')";
                                $result5=$obj->executequery($um);
								...
								while($urow=mysqli_fetch_array($result5)){
									?>
